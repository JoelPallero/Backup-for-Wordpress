...
class DN325_Backup_Ajax {
    ...
    public static function handle_list() {
        ...
        // Obtener solo backups válidos (con token correcto)
        $valid_backups = DN325_Backup_License::get_valid_backups();
        $backups = [];

        foreach ($valid_backups as $file) {
    ...
    public static function handle_get_progress() {
        ...
        if (file_exists($log_file)) {
            $lines = DN325_Backup_Logger::get_recent_logs(100);
            foreach ($lines as $line) {
                // Extraer solo líneas INFO que contengan información de progreso
                if (preg_match('/\[INFO\].*?(Procesando tabla|Exportando|Creando|Base de datos|wp-content|completado|Tabla \d+:|Lote \d+\/)/', $line)) {
                    $progress[] = trim($line);
                }
            }
        }

        wp_send_json_success(['progress' => array_slice($progress, -20)]); // Últimas 20 líneas
    }
