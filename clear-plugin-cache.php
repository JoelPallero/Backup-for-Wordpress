...
echo '<h1>Limpieza de Cache del Plugin Nabi Backup</h1>';
...
// Limpiar transients
global $wpdb;
$wpdb->query("DELETE FROM {$wpdb->options} WHERE option_name LIKE '_transient_Nabi_%'");
$wpdb->query("DELETE FROM {$wpdb->options} WHERE option_name LIKE '_transient_timeout_Nabi_%'");

echo '<p>✅ Transients eliminados</p>';

// Limpiar cache de plugins activos
$active_plugins = get_option('active_plugins', []);
$plugin_file = 'backup_for_wp/Nabi-backup-for-wordpress.php';

// Si el plugin está en la lista con otro nombre, eliminarlo
$updated = false;
foreach ($active_plugins as $key => $plugin) {
    if (strpos($plugin, 'Nabi-backup') !== false && $plugin !== $plugin_file) {
